...
public class ImageTransform {
	 ...
	 public BufferedImage getRotatedImage(BufferedImage img, double angle) {
	        return getRotatedImage(img, img.getWidth() / 2, img.getHeight() / 2, angle);
	    }
	 
	    public BufferedImage getRotatedImage(BufferedImage img, int rotationCenterX, int rotationCenterY, double angle) {
	 
	        BufferedImage image = initRotatedImage(img, rotationCenterX, rotationCenterY, angle);
	        int rotatedX;
	        int rotatedY;
	        int maxY = -99999;
	        int maxX = -99999;
	        ...
	        for (int x = 0; x < img.getWidth(); x++) {
	            for (int y = 0; y < img.getHeight(); y++) {
	                rotatedX = centerX + calculateRotatedX(x - rotationCenterX, y - rotationCenterY, angle);
	                rotatedY = centerY + calculateRotatedY(x - rotationCenterX, y - rotationCenterY, angle);
	                if (maxX < rotatedX) {
	                    maxX = rotatedX;
	                }
	                if (maxY < rotatedY) {
